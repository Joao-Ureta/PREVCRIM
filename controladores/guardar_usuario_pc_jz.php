<!-- CONTROLADOR INGRESAR USUARIO PAZ CIUDADANA -->
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['nombre_completo'], $_POST['rut'], $_POST['correo'], $_POST['contrasena'], $_POST['rol'])) {
        ...
        $roles_permitidos = ['Operador', 'JefeZona'];
        ...
        if ($stmt->execute()) {
            echo "<script>alert('$rol registrado exitosamente'); window.location.href='../vistas/instituciones/Paz_ciudadana/ingresar_usuario.php';</script>";
        } else {
            echo "<script>alert('Error al registrar $rol: " . $stmt->error . "'); window.history.back();</script>";
        }
    ...
    echo "<script>alert('Acceso inválido'); window.location.href='../vistas/instituciones/Paz_ciudadana/ingresar_usuario.php';</script>";
}
?>
